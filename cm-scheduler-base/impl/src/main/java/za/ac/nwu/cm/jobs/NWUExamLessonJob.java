...
/**
 * This is a quartz job that creates a Exam Lesson plan in Course Sites
 *
 * @author JC Gillman
 * 
 */
@Slf4j
public class NWUExamLessonJob implements Job {
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		log.info("NWUExamLessonJob - execute start");
		...
		try {
			getService().updateNWUExamLessons(previousFireTime == null ? null : previousFireTime);
		} catch (JobExecutionException e) {
			throw new JobExecutionException("NWUExamLessonJob failed");
		}
		
		log.info("NWUExamLessonJob - execute finished");
	}
