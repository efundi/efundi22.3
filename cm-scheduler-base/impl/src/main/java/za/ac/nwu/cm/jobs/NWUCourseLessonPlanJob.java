...
/**
 * This is a quartz job that creates a Lesson plan in Course Sites
 *
 * @author JC Gillman
 * 
 */
@Slf4j
public class NWUCourseLessonPlanJob implements Job {
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		log.info("NWUCourseLessonPlanJob - execute start");
		...
		try {
			getService().updateNWUCourseLessonPlans(previousFireTime == null ? null : previousFireTime);
		} catch (JobExecutionException e) {
			throw new JobExecutionException("NWUCourseLessonPlanJob failed");
		}
		
		log.info("NWUCourseLessonPlanJob - execute finished");
	}
